...
function Author() {
  ...
  useEffect(() => {
    ...
    if (walletAdd)
      fetchOwnedItems();
  }, [currPageforOwned, priceSort1, searchOnEnter1, searchFor1, ERCType1, walletAdd])
  ...
  useEffect(() => {
    ...
    if (walletAdd)
      fetchOnSaleItems();
  }, [currPageforOnSale, priceSort2, searchOnEnter2, searchFor2, ERCType2, walletAdd])

  const fetch = async () => {
    setLoader(true);
    let _profile = await GetIndividualAuthorDetail({ userID: id });
    setProfile(_profile);
    setWalletAdd(_profile?.walletAddress?.toLowerCase());
                  ...
                  ownedNFTs && ownedNFTs?.length > 0 ? ownedNFTs?.map((card, key) => {
                    return (<div className={grid} key={key}>
                      <AuthorListing
                        ...
                        flag={cookies?.selected_account?.toLowerCase() === walletAdd?.toLowerCase() ? "mynft" : ""}
                      />
                    </div>)
                  }
                  ) : (
